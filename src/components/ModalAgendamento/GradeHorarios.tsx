...
interface GradeHorariosProps {
    ...
    carregando: boolean;
    /** O pai lida com o fechamento do modal após este callback. */
    onConfirmar: (dataIso: string, hora: string) => void;
}

export function GradeHorarios({
    diaSelecionado, mesAtual, anoAtual, horariosDoDia, carregando, onConfirmar,
}: GradeHorariosProps) {
    const nomesMeses = [
    ...
    if (!diaSelecionado) {
        return (
            <p className="text-sm text-center text-gray-400 font-light py-6">
    ...
    if (carregando) {
        return (
            <p className="text-sm text-center text-caramelo font-light py-6 animate-pulse">
                Buscando agenda da equipe…
            </p>
        );
    }
    ...
    if (horariosDoDia.length === 0) {
        return (
            <div className="text-center py-4 bg-red-50 rounded-lg border border-red-100 mt-2">
                <p className="text-sm text-red-500 font-medium">Sem horários disponíveis</p>
                <p className="text-xs text-red-400 mt-1">
                    Tente outro dia ou remova algum serviço.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-3">
            <h3 className="text-xs font-semibold tracking-widest uppercase text-gray-500 text-center">
                Horários livres · {diaSelecionado} de {nomesMeses[mesAtual]}
            </h3>
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-[180px] overflow-y-auto pr-1">
                {horariosDoDia.map(hora => {
                    ...
                    return (
                        <button
                            key={hora}
                            onClick={() => onConfirmar(dataIso, hora)}
