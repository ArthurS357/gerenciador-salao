...
export default function PainelEstoquePage() {
    ...
    const [formData, setFormData] = useState({
        nome: '', descricao: '', precoCusto: 0, precoVenda: 0, estoque: 0
    });
    ...
    const handleSalvarProduto = async (e: React.FormEvent) => {
        ...
        if (res.sucesso) {
            ...
            carregarProdutos(); // Recarrega a tabela em tempo real
        } else {
            setMensagem({ texto: res.erro || 'Erro ao salvar.', tipo: 'erro' });
        }
    };

    const handleAjusteRapido = async (id: string, quantidade: number) => {
        const res = await ajustarEstoque(id, quantidade);
        if (res.sucesso) carregarProdutos();
        else alert(res.erro);
    };
        ...
        <div className="min-h-screen bg-[#fdfbf7] p-8 font-sans">
            ...
            <section className="bg-white rounded-lg shadow overflow-hidden border border-[#e5d9c5]">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                        {produtos.length === 0 ? (
                            <tr><td colSpan={5} className="p-8 text-center text-gray-500">Nenhum produto cadastrado no estoque.</td></tr>
                        ) : (
                            produtos.map(p => (
                                <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50">
                                    ...
                                    <td className="p-4 text-center font-bold text-green-600">{(p.precoVenda).toFixed(2)}</td>
                                    <td className="p-4 text-center">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${p.estoque <= 5 ? 'bg-red-100 text-red-700' : 'bg-gray-200 text-gray-800'}`}>
                                            {p.estoque} un
                                        </span>
                                    </td>
                                    <td className="p-4 text-right space-x-2">
                                        <button onClick={() => handleAjusteRapido(p.id, -1)} className="bg-red-50 text-red-600 px-3 py-1 rounded border border-red-200 hover:bg-red-100 font-bold">- 1</button>
                                        <button onClick={() => handleAjusteRapido(p.id, 1)} className="bg-green-50 text-green-600 px-3 py-1 rounded border border-green-200 hover:bg-green-100 font-bold">+ 1</button>
                                    </td>
                                </tr>
                            ))
                        )}
                    ...
                    <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg border-t-4 border-[#5C4033]">
                        ...
                        <form onSubmit={handleSalvarProduto} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" className="w-full border rounded px-3 py-2" onChange={e => setFormData({ ...formData, descricao: e.target.value })} />
                            </div>
                            <div className="grid grid-cols-3 gap-4">
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Custo (R$)</label>
                                    <input required type="number" step="0.01" className="w-full border rounded px-3 py-2" onChange={e => setFormData({ ...formData, precoCusto: Number(e.target.value) })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Venda (R$)</label>
                                    <input required type="number" step="0.01" className="w-full border rounded px-3 py-2" onChange={e => setFormData({ ...formData, precoVenda: Number(e.target.value) })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Qtd Inicial</label>
                                    <input required type="number" className="w-full border rounded px-3 py-2" onChange={e => setFormData({ ...formData, estoque: Number(e.target.value) })} />
                                </div>
                            </div>
                            <div className="flex justify-end gap-3 mt-6">
                                ...
                                <button type="submit" className="px-4 py-2 bg-[#5C4033] text-white font-bold rounded hover:bg-[#3e2b22]">Salvar Produto</button>
                            </div>
                        </form>
                    </div>
                </div>
